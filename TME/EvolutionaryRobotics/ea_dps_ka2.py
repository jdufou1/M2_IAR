...
def launch_ea(mu=100, lambda_=200, cxpb=0.3, mutpb=0.7, ngen=100, verbose=False, resdir="res"):
    ...
    ---
    """
    list_global_fitness = list()
    fitness_reached = None
    """
    ---
    """
    random.seed()
    ...
    current_value = log['dist_end_effector']
    ...
    for gen in tqdm(range(1, ngen + 1)):
        ...
        print("a : ",a)
        v, t , inters = a.fw_kinematics(best_angles)
        print(v,t,inters)
        x_ = [p[0] for p in t]
        y_ = [p[1] for p in t]
        plt.figure()
...
#         print(population)
        # print([individu.fitness.values for individu in population])
        ...
        if (newvaluemin<valuemin):
            ...
            eval_ka(pq[indexmin],
                registered_envs[env_name]['eval_params']["lengths"],
                registered_envs[env_name]['eval_params']["target_pos"],
                registered_envs[env_name]['eval_params']["walls"],
                True,"gen%04d"%(gen)
            )
        """
        ########################################################################
    ...
    if fitness_reached == None :
        fitness_reached = ngen
    
    return population, None, paretofront, grid,list_global_fitness,fitness_reached
...
if (__name__ == "__main__"):
    ...
    for test in range(nb_tests) : 
        ...
        _, _, _, _,list_global_fitness,fitness_reached = launch_ea(mu=mu, lambda_=lambda_, ngen=ngen, resdir=resdir)
        if fitness_reached == ngen :
            fails += 1
        else : 
            success += 1
            list_result_generation.append(fitness_reached)
        
    list_result_generation = np.array(list_result_generation)

    print(f"resultats success : {success}/{success+fails} = {success/(success+fails)*100}%")
    print(f"resultats fails : {fails}/{success+fails} = {fails/(success+fails)*100}%")
    if len(list_result_generation) != 0 :
        print(f"resultats convergence moyenne de {list_result_generation.mean()} sur {len(list_result_generation)} echantillons")
    ...
    plt.figure()
    plt.title("fitness en fonction de la generation")
    plt.plot(list_global_fitness)
    plt.xlabel("generation")
    plt.ylabel("fitness")
    plt.savefig(resdir+".png")
