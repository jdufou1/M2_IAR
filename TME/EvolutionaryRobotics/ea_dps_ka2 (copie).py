...
ea_generic={
    ...
    'nb_gen': 500, # number of generations
...
if (__name__ == "__main__"):
    ...
    print(f"resultats reussite : {success}/{success+fails} = {success/(success+fails)*100}%")
    print(f"resultats rate : {fails}/{success+fails} = {fails/(success+fails)*100}%")
    if len(list_result_generation) != 0 :
        print(f"Convergence moyenne de {list_result_generation.mean()} génération sur {len(list_result_generation)} echantillons")
    
    """
    
    
    """
    Test simple pour sauvegarder les configurations
    """
    """
    resdir="res_"+env_name+"_"+datetime.datetime.now().strftime("%Y_%m_%d_%H:%M:%S")
    os.mkdir(resdir)
