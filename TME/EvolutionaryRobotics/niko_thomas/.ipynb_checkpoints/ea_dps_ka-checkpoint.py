...
ea_generic={
    ...
    'nov_add_strategy': "random", # archive addition strategy (either 'random' or 'novel' or 'novel+local')
...
def get_random_x_y(grid):
    x = random.randint(0, 99)
    y = random.randint(0, 99)
    while not (x, y) in grid:
        x = random.randint(0, 99)
        y = random.randint(0, 99)
    return (x, y)

def map_elites_or(grid, toolbox, cxpb):
    p = random.uniform(0, 1)
    (x, y) = get_random_x_y(grid)
    if p <= cxpb: # croisement
        (x2, y2) = get_random_x_y(grid)
        child1, child2 = [toolbox.clone(ind) for ind in (grid[(x, y)], grid[(x2, y2)])]
        (ind3, _) = tools.cxBlend(child1, child2, 0.5)
...
def var_or_map_elites(grid, toolbox, lambda_, cxpb):
    offspring = []
    for _ in range(lambda_):
        ind = map_elites_or(grid, toolbox, cxpb)
        grid = grid_management.add_to_grid(grid, ind, None)
        offspring.append(ind)
    return offspring, grid


def launch_ea(mu=100, lambda_=200, cxpb=0.3, mutpb=0.7, ngen=100, verbose=False, resdir="res", map_elites=False):
    ...
    for ind, (fit, bd, log) in zip(invalid_ind, fitnesses_bds):
        ...
        ffit.write(str(fit)+"\n")
        ffit.flush()
        grid = grid_management.add_to_grid(grid,ind, fit, 
    ...
    archive, ranks = updateNovelty(population,population,None,
    ...
    for i, ind in enumerate(population):
        if (registered_envs[env_name]['selection']=="FIT+NS"):
            ...
            ind.fitness.values=(ind.novelty, ranks[i])
        if best is None:
            best = ind.log[registered_envs[env_name]['watch_max']]
        _max = max(best, ind.log[registered_envs[env_name]['watch_max']])
    ...
    for gen in tqdm(range(1, ngen + 1)):
        ...
        if map_elites:
            offspring, grid = var_or_map_elites(grid, toolbox, lambda_, cxpb)
        else:
        ...
        for ind, (fit, bd, log) in zip(invalid_ind, fitnesses_bds):
            ...
            grid = grid_management.add_to_grid(grid, ind, ind.fit, 
                                        dim=registered_envs[env_name]['dim_grid'], 
                                        min_v=registered_envs[env_name]['grid_min_v'], 
                                        max_v=registered_envs[env_name]['grid_max_v'])
        ...
        archive, ranks = updateNovelty(pq,offspring,archive,
                              registered_envs[env_name]['nov_k'],
                              registered_envs[env_name]['nov_add_strategy'],
                              registered_envs[env_name]['nov_lambda'])
