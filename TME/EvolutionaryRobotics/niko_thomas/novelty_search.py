...
class NovArchive:
    ...
    def __init__(self, lbd, k=15, fits=[]):
        self.all_bd=lbd
        self.fit = fits
...
def updateNovelty(population, offspring, archive, k=15, add_strategy="random", _lambda=6, verbose=False):
    ...
    if(add_strategy=="random"):
        ...
        soff = sorted(offspring, key=functools.cmp_to_key(getnov))
    ...
    elif(add_strategy == "novel+local"):
        ranks = np.zeros(len(population))
        if (archive) and (archive.size() >= k):
            for i in range(len(population)):
                rank = k
                for j in range(k):
                    if population[i].fit < archive.fit[j]:
                        rank -= 1
                ranks[i] = rank
        # print(population)
        #soff=sorted(offspring,lambda x:x.novelty)
        soff = sorted(offspring, key=functools.cmp_to_key(getnov))
        # print([n.novelty for n in soff])
        ilast=len(offspring)-_lambda
        lbd=[soff[i].bd for i in range(ilast,len(soff))]
    else:
        print("ERROR: updateNovelty: unknown add strategy(%s), valid alternatives are \"random\", \"novel\" and \"novel+local\""%(add_strategy))
        return None
        
    if(archive==None):

        archive=NovArchive(lbd,k, fits=[ind.fit for ind in population])
    else:
        archive.update(lbd)

    if add_strategy == "novel+local":
        return archive, ranks
    else:
        return archive, 0
